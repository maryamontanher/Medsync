...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nome || !$telefone || !$sexo || !$data_nascimento) {
        ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
            $pasta = __DIR__ . '/uploads/pacientes/';
            if (!is_dir($pasta)) {
                mkdir($pasta, 0755, true);
            }
            $extensao = strtolower(pathinfo($_FILES['foto']['name'], PATHINFO_EXTENSION));
            $nome_foto = 'paciente_' . $pmaior_id . '_' . time() . '.' . $extensao;
        ...
        if (!$erro) {
          $campos_update = ["pmaior_nome = ?", "pmaior_telefone = ?", "pmaior_sexo = ?", "pmaior_datanasc = ?"];
          $tipos_bind = "ssss";
          $valores_bind = [&$nome, &$telefone, &$sexo, &$data_nascimento];
      
          if ($nome_foto) {
              $campos_update[] = "pmaior_foto = ?";
              $tipos_bind .= "s";
              $valores_bind[] = &$nome_foto;
          }
      
          if ($alterar_senha) {
              $stmt = $mysqli->prepare("SELECT pmaior_senha FROM paciente_maior WHERE pmaior_id = ?");
          ...
          if (!$erro) {
            $sql = "UPDATE paciente_maior SET " . implode(", ", $campos_update) . " WHERE pmaior_id = ?";
            $stmt = $mysqli->prepare($sql);
    
            $tipos_bind .= "i";
            
            $valores_bind[] = &$pmaior_id;
...
$stmt = $mysqli->prepare("SELECT pmaior_nome, pmaior_telefone, pmaior_endereco, pmaior_sexo, pmaior_datanasc, pmaior_email, pmaior_cpf, pmaior_estadocivil, pmaior_foto FROM paciente_maior WHERE pmaior_id = ?");
$stmt->bind_param("i", $pmaior_id);
$stmt->execute();
...
<head>
    ...
    <title>Medsync | Perfil do Paciente</title>
...
<nav class="navbar">
  ...
  <a href="painelpaciente.php"><i class="fa-solid fa-arrow-left"></i> Voltar ao painel</a>
</nav>
